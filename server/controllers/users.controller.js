...
const registerUser = asyncHandler(async (req, res) => {
  validateFields(["fullname", "password", "username"], req.body);
...
const loginUser = asyncHandler(async (req, res) => {
  validateFields(["username", "password"], req.body);
  ...
  if (!existingUser)
    throw new ApiErrorResponse(403, "User not found, please register!");
...
const refreshAccessToken = asyncHandler(async (req, res) => {
  ...
  if (!user)
    throw new ApiErrorResponse(401, "Session expired, please login again!");
...
const changeCurrentPassword = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, existingUser, "Password updated successfully"));
});
...
const updateUserAvatar = asyncHandler(async (req, res) => {
  if (!req.files.avatar?.[0])
    throw new ApiErrorResponse(400, "Please upload an avatar file");

  const avatar = await uploadFile(req.files.avatar[0].path);
  if (!avatar)
    throw new ApiErrorResponse(500, "Something went wrong uploading the image");
...
const updateUserCover = asyncHandler(async (req, res) => {
  if (!req.files.coverImage?.[0])
    throw new ApiErrorResponse(400, "Please upload a cover image file");
  ...
  if (!updatedUser)
    throw new ApiErrorResponse(500, "Unable to update cover image");
